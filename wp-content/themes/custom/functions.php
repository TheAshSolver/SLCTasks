...
add_shortcode('graphql_clubs', function() {
    ...
    $output .= 'Hello <div class="clubs-grid">';
    foreach ($clubs as $club) {
        $url = "https://clubs.iiit.ac.in/clubs/" . esc_attr($club['cid']);
        $output .= '<div class="club-card">';
        $output .= '<a href="' . "hello". $url . '" class="club-name" target="_blank">' . esc_html($club['name']) . '</a>';
...
add_shortcode('graphql_events', function() {
    ...
    // 1. Fetch Events from GraphQL
    $response = wp_remote_post('http://nginx/graphql', [
        'headers' => ['Content-Type' => 'application/json'],
        'body'    => json_encode(['query' => '{ events { name location clubid } }']),
        'timeout' => 15
    ]);
    ...
    // DEBUG: Count total data points returned
    $total_count = count($events);
    $output = '<div style="background:#eee; padding:10px; margin-bottom:20px; border-left:5px solid #103016;">';
    

    // 2. Search Area
    $output .= '<div class="search-area">
        <form method="POST" action="' . esc_url( get_permalink() ) . '">
            <input type="text" name="event_search" placeholder="Search event names..." value="' . esc_attr($search_term) . '" style="padding:10px; width:250px;">
    ...
    // 3. Loop and Filter
    $display_count = 0;
    $event_list_html = '<div class="overall" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px;">';
    ...
    foreach ($events as $event) {
        ...
        $display_count++;
        $event_list_html .= "<div class='card' style='border: 1px solid #252020; padding: 20px; border-radius: 8px; background: #ebc33fff; '>";
        $event_list_html .= "<strong>" . esc_html($event['name']) . "</strong><br>";
        $event_list_html .= "<small>Club ID: " . esc_html($event['clubid']) . "</small>";
        $event_list_html .= "</div>";
    }
    $event_list_html .= '</div>';
